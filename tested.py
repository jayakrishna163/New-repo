...
from pyspark.sql import SparkSession
from pyspark.sql.types import StringType, IntegerType, DateType, TimestampType, StructType, StructField
from pyspark.sql.functions import col, dense_rank, sum as spark_sum, length, row_number
from pyspark.sql.window import Window
...
class ETLProcessor:
    ...
    def get_next_row_num(self, table_name, metric_name, as_of_date):
        # Query to get the next row number for the specified table and metric
        row_num_query = f"""
    ...
    def log_quality_control_metrics(self, table_name, metric_name, metric_flag, metric_numeric, sample_data, as_of_date, is_anomaly):
        ...
        # Fetch next row number
        next_row_num = self.get_next_row_num(table_name, metric_name, as_of_date)

                    ...
                    if metric_name in metrics_mapping:
                        ...
                        if metric_flag == "numeric":
                            ...
                            if current_sample_data is None:
                                is_anomaly = "Yes"
                            elif len(current_sample_data) < 5 :
                                is_anomaly = "Yes"
...
df1 = spark.sql("select * from etl_test.rpt2")
df1.show()

# COMMAND ----------

df2 = spark.sql("select * from etl_test.acl3")
df2.show()
...
from pyspark.sql import SparkSession
from pyspark.sql.types import StringType, IntegerType, DateType, TimestampType, StructType, StructField
from pyspark.sql.functions import col, dense_rank, sum as spark_sum, length
from pyspark.sql.window import Window
    ...
    def detect_anomalies(self, rpt_table, rows_inserted_count, metric1_sum, metric2_sum, sample_data, end_time):
        try:
            ...
            if anomaly_metrics.count() == 0:
                ...
                sample_data_list = sample_data.select("name").rdd.flatMap(lambda x: x).collect()
                for data in sample_data_list:
                    is_anomaly = "Yes" if len(data) > 5 else "No"
                    if is_anomaly == "Yes":
                        self.log_quality_control_metrics(rpt_table, "sample_data", "varchar", len(data), data, end_time.date(), is_anomaly)
                        break  # Stop after the first anomaly
            else:
                window_spec = Window.partitionBy("metric_name").orderBy(col("update_ts").desc())
                latest_metrics = anomaly_metrics.withColumn("rank", dense_rank().over(window_spec)) \
                                                .filter(col("rank") == 1) \
...
if __name__ == "__main__":
    ...
    try:
        ...
        sample_data = rpt_df.select("name").rdd.flatMap(lambda x: x).collect()) if rpt_df.count() > 0 else None
...
class ETLProcessor:
    ...
    def get_next_row_num(self, table_name, as_of_date):
        # Query to get the next row number for the specified table
        row_num_query = f"""
            SELECT IFNULL(MAX(row_number), 0) + 1 AS next_row_num 
            FROM etl_test.managed_quality_control_table1
            WHERE table_name = '{table_name}'
            AND as_of_date = '{as_of_date}'
        """
        # Execute the query and retrieve the next row number
        next_row_num_df = self.spark.sql(row_num_query)
        next_row_num = next_row_num_df.collect()[0]["next_row_num"]
        return next_row_num
    ...
    def log_quality_control_metrics(self, table_name, metric_name, metric_flag, metric_numeric, sample_data, as_of_date, is_anomaly):
        ...
        # Fetch next row number
        next_row_num = self.get_next_row_num(table_name, as_of_date)
    ...
    def detect_anomalies(self, rpt_table, rows_inserted_count, metric1_sum, metric2_sum, sample_data, as_of_date, end_time):
        try:
            anomaly_query = f"""
                SELECT * FROM etl_test.managed_quality_control_table1
                WHERE metric_name IN ('row_count', 'metric1_sum', 'metric2_sum', 'sample_data')
...
if __name__ == "__main__":
    ...
    try:
        ...
        etl_processor = ETLProcessor(spark)
        etl_processor.detect_anomalies(rpt_table, rows_inserted_count, sum_metrics1, sum_metrics2, sample_data, as_of_date, end_time)
    except Exception as e:
        print(f"Error in ETL Job: {e}")
...
class ETLProcessor:
    ...
    def get_next_row_num(self, table_name, as_of_date):
        # Query to get the next row number for the specified table
        row_num_query = f"""
            SELECT COALESCE(MAX(row_number), 0) + 1 AS next_row_num 
            FROM etl_test.managed_quality_control_table1
            WHERE table_name = '{table_name}'
            AND as_of_date = '{as_of_date}'
        """
    ...
    def detect_anomalies(self, rpt_table, rows_inserted_count, metric1_sum, metric2_sum, sample_data, end_time):
        try:
            # Query to fetch latest metrics from managed_quality_control_table1
            anomaly_query = f"""
                SELECT * FROM etl_test.managed_quality_control_table1
                WHERE metric_name IN ('row_count', 'metric1_sum', 'metric2_sum', 'sample_data')
                AND table_name = '{rpt_table}'
                AND as_of_date = '{end_time.date()}'
            """

            anomaly_metrics = self.spark.sql(anomaly_query)

            # If no metrics are found for the current date, log them as anomalies
            if anomaly_metrics.count() == 0:
                self.log_quality_control_metrics(rpt_table, "row_count", "numeric", rows_inserted_count, None, end_time.date(), "Yes" if rows_inserted_count == 0 else "No")
                self.log_quality_control_metrics(rpt_table, "metric1_sum", "numeric", metric1_sum, None, end_time.date(), "Yes" if metric1_sum == 0 else "No")
                self.log_quality_control_metrics(rpt_table, "metric2_sum", "numeric", metric2_sum, None, end_time.date(), "Yes" if metric2_sum == 0 else "No")
                ...
                for data in sample_data_list:
                    if len(data) > 5:
                        self.log_quality_control_metrics(rpt_table, "sample_data", "varchar", len(data), data, end_time.date(), "Yes")
                        break  # Stop after the first anomaly
            else:
                # Determine the next row number
                next_row_num = self.get_next_row_num(rpt_table, end_time.date())

                # Process each metric from the anomaly_metrics query
                for row in anomaly_metrics.collect():
                    metric_name = row["metric_name"]
                    metric_flag = row["metric_flag"]
                    current_metric_numeric = row["metric_numeric"]
                    current_sample_data = row["sample_data"]

                    if metric_name == "row_count":
                        is_anomaly = "Yes" if rows_inserted_count == 0 else "No"
                        self.log_quality_control_metrics(rpt_table, "row_count", "numeric", rows_inserted_count, None, end_time.date(), is_anomaly)
                    elif metric_name == "metric1_sum":
                        is_anomaly = "Yes" if metric1_sum == 0 else "No"
                        self.log_quality_control_metrics(rpt_table, "metric1_sum", "numeric", metric1_sum, None, end_time.date(), is_anomaly)
                    elif metric_name == "metric2_sum":
                        is_anomaly = "Yes" if metric2_sum == 0 else "No"
                        self.log_quality_control_metrics(rpt_table, "metric2_sum", "numeric", metric2_sum, None, end_time.date(), is_anomaly)
                    elif metric_name == "sample_data":
                        is_anomaly = "Yes" if current_sample_data is None or len(current_sample_data) > 5 else "No"
                        self.log_quality_control_metrics(rpt_table, "sample_data", "varchar", len(current_sample_data), current_sample_data, end_time.date(), is_anomaly)
...
if __name__ == "__main__":
    ...
    try:
        ...
        acl_df.createOrReplaceTempView("acl3_temp")
        rpt_df = spark.sql("""
            SELECT *, age - 2 AS age_2years_ago
            FROM acl3_temp
        """)
